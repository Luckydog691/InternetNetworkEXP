...
std::map<std::string, std::string[3]>m;
...
void savefile()
{
	std::ofstream fout("test.txt", std::ios::out);
	for (auto e = m.begin(); e != m.end(); e++)//遍历map
	{
		fout << "FTP:" << e->first << " USR:" << e->second[0] << " PAS:" << e->second[1]
			<< " STU:" << e->second[2] << std::endl;
	}
	fout.close();
}
void packet_handler(u_char* param, const struct pcap_pkthdr* header, const u_char* pkt_data)
{
	...
	if (command == "230 " || command == "530 ")//登录成功/失败
	{
		...
		if (command == "230 ")//登录成功
		{
			...
			fout << "FTP:" << dstipstr << " USR:" << m[dstipstr][0] << " PAS:" << m[dstipstr][1]
				<< " STU:" << m[dstipstr][2] << std::endl;
		}
		if (command == "530 ")//登录失败
		{
